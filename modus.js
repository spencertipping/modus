...
//     my_list.val()                                               // -> [true, false, false]
    ...
    it.list(new_element) = this.modus(given.nothing in this.children() *[$(x).val()] /seq,
                                      given.xs      in this.empty() -se- xs *![it.append(new_element().val(x))] /seq),
...
//   You can use ambiguous paths; if you do this, the first matching element will be chosen.
    ...
    it.composite(paths) = this.modus(given.nothing in paths /keys *[[x, find(this, paths[x]).eq(0).val()]] /object /seq,
                                     given.xs      in paths /keys *![find(this, paths[x]).eq(0).val(xs[x])] /seq -re- this),

    where [find(container, path) = container.filter(path).add(container.find(path))]]})(jQuery);
